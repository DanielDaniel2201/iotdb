  ...
  public void addInput(Column[] column, BitMap bitMap, int lastIndex) {
    if (column[1] instanceof RLEColumn) {
      ...
      while (curIndex < lastIndex) {
        ...
        if ((!curPattern.mayHaveNull()) && (bitMap == null || bitMap.getRegion(curIndex, curPatternLength).isAllMarked())) {
          countValue += curPatternLength;
          curIndex += curPatternLength;
        } else {
          for (int j = 0; j < curPatternLength; j++, curIndex++) {
            if ((!curPattern.isNull(j)) && (bitMap == null || bitMap.isMarked(curIndex))) {
              countValue++;
            }
            continue;
          }
